...
export class ClientesComponent implements OnInit {
  ...
  spinnerEnderecos: boolean = false;
  spinnerPedidos: boolean = false;
  ...
  displayed = ['image', 'nome', 'phone', 'criado', 'acoes'];
  ...
  isOpenView(view: number, id: any) {
    ...
    if(view === 4){
      this.getByID(id);
      this.enderecosCliente(id);
      this.pedidosCliente(id);
    }
  ...
  get() {
    ...
    this.crudSrv.get('clientes').subscribe(
    res => {
      this.data = res;
    ...
    //Reseta o formuulário de cadastrado
    this.dataForm.reset();
  }

  getByID(id: number){
    this.crudSrv.getByID('clientes', id).subscribe(
      res => {
  ...
  post() {
    ...
    this.crudSrv.post('clientes', this.dataForm.value).subscribe(
      res => {
  ...
  update(id: number){
    ...
    this.crudSrv.update('clientes', id, this.updateForm.value).subscribe(
      res => {
  ...
  openModalDelete(id: number){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result === true){
        ...
        this.crudSrv.delete('clientes', id).subscribe(
          res => {
  ...
  //Histórico de pedidos e endereços do cliente
  enderecosCliente(id: number) {
    this.spinnerEnderecos = true;

    this.crudSrv.getByID('cliente/enderecos', id).subscribe(
      (res: any) => {
        if(res.length <= 0){
          this.enderecos = null;
          this.spinnerEnderecos = false;
          return;
        }
  ...
  pedidosCliente(id: number) {
    this.spinnerPedidos = true;

    this.crudSrv.getByID('cliente/historico', id).subscribe(
      (res: any) => {
        if(res.length <= 0){
          this.pedidos = null;
          this.spinnerPedidos = false;
          return;
        }
