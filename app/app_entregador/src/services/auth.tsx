...
interface AuthContextData {
  ...
  signOut(): void;
  updateUser(id: number, name: string, name_array: any, email: string): void;
}
...
const AuthProvider: React.FC = ({children}) => {
  ...
  async function signIn(email: string, password: string) {
    ...
    try {
     api.post('auth/entregador/login', {email: email, password: password}).then(async (user: any) => {
        api.defaults.headers = {
  ...
  function updateUser(id: number, name: string, name_array: any, email: string){
    let user = {
      id: id,
      name: name,
  ...
  async function signOut() {
    ...
    await api.post(`auth/entregador/logout/${user?.id}`, token).then(async res => { 
